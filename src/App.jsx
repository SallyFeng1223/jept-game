...
const RAW_QUESTIONS = [
  ...
  [
    'picture',
    '👒',
    'This is my hat. What color is it?',
    'It is orange.',
    'It is blue.',
    'It is ten.',
    1,
  ...
  [
    'conversation',
    '💬',
    'Are there three glasses?',
    'Yes, there are three glasses.',
    'No, there is not a plate.',
    'It is a glass.',
    0,
  ],
  [
  ...
  [
    ...
    '👦',
    'Who is he?',
    'He is my friend. His name is Leo.',
    'She is Nini.',
    'I am Tom.',
    0,
  ],
...
const DEFAULT_QUESTIONS = RAW_QUESTIONS.map((q, index) => {
  ...
  return {
    ...
    playAllText:
      q[0] === 'picture'
        ? `Look at the picture. A. ${q[3]} B. ${q[4]} C. ${q[5]}`
        : `Listen carefully. ${q[2]}`,
...
export default function App() {
    ...
    const loadVoices = () => {
      ...
      if (voices.length > 0) {
        const usVoices = voices.filter(
          (v) => v.lang.includes('US') && v.lang.includes('en')
        );
        ...
        if (mixed.length === 0)
          mixed.push(...voices.filter((v) => v.lang.startsWith('en')));
        setVoicesPool(mixed);
      }
    };
  ...
  const finishGame = async (finalScore) => {
    ...
    [523, 659, 783, 1046].forEach((freq, i) => {
      setTimeout(
        () => playTone(audioCtxRef.current, freq, 'square', 0.15),
        i * 150
      );
    });
    ...
    reader.onload = (e) => {
      try {
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          if (parts.length >= 7) {
            const typeId =
              parts[0] === '1' || String(parts[0]).includes('看圖')
                ? 'picture'
                : 'conversation';
            ...
            if (!options.includes(rawCorrectAns)) {
              const closeMatch = options.find(
                (o) =>
                  o.toLowerCase().trim() === rawCorrectAns.toLowerCase().trim()
              );
              if (closeMatch) {
            ...
            newQuestions.push({
              ...
              playAllText:
                typeId === 'picture'
                  ? `Look at the picture. A. ${parts[3]} B. ${parts[4]} C. ${parts[5]}`
                  : `Listen carefully. ${parts[2]}`,
              playOptionTexts: options,
              options: options,
        ...
        if (newQuestions.length > 0) {
          ...
          localStorage.removeItem('cept_unplayed_pool');
          alert(
            `成功匯入 ${newQuestions.length} 題，並已完成答案自動檢核修復！`
          );
        } else {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-indigo-50 p-4">
        <div className="bg-white rounded-3xl shadow-xl p-8 max-w-md w-full text-center border-4 border-indigo-200">
          ...
          <h1 className="text-3xl font-black text-indigo-800 mb-2">
            聽力特訓班
          </h1>
          <p className="text-gray-600 mb-6 font-medium bg-gray-100 py-1 px-4 rounded-full inline-block">
            混合口音實戰模擬
          </p>
          ...
          <button
            ...
            className={`w-full text-white text-xl font-bold py-4 px-8 rounded-2xl transition-transform transform hover:scale-105 flex items-center justify-center gap-3 shadow-lg mb-4 ${
              ttsSupported ? 'bg-indigo-500 hover:bg-indigo-600' : 'bg-gray-400'
            }`}
          >
          ...
          <div className="pt-4 border-t-2 border-dashed border-gray-200 text-left">
            ...
            <div className="bg-gray-50 rounded-lg p-3 text-xs text-gray-500 mb-2 shadow-inner">
              ...
              <ul className="list-disc pl-4 space-y-1">
                ...
                <li>
                  可使用 <strong>Excel</strong> 編輯，完成後儲存為{' '}
                  <strong>CSV 格式</strong>。
                </li>
                <li>
                  題型請填：
                  <span className="text-indigo-500 font-bold">1 (看圖)</span> 或{' '}
                  <span className="text-indigo-500 font-bold">2 (對話)</span>。
                </li>
              </ul>
            </div>

            <p className="text-xs text-center text-gray-400 font-bold mt-2">
              目前載入: {questionsPool.length} 題
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col items-center justify-center min-h-screen bg-green-50 p-4">
        <div className="bg-white rounded-3xl shadow-xl p-8 max-w-md w-full text-center border-4 border-green-200 overflow-hidden flex flex-col max-h-[95vh]">
          <div className="flex-shrink-0">
            ...
            <div className="text-7xl font-black text-green-500 mb-6 drop-shadow-sm">
              {score}
            </div>
          </div>
            ...
            {mistakes.length === 0 ? (
              ...
              <div>
                <h3 className="font-bold text-gray-700 mb-3 flex items-center gap-2">
                  <XCircle className="w-5 h-5 text-red-500" /> 錯題回顧 (
                  {mistakes.length}題)
                </h3>
                <div className="space-y-3">
                  {mistakes.map((m, idx) => (
                    <div
                      key={idx}
                      className="bg-red-50 p-3 rounded-xl border border-red-100 text-sm"
                    >
                      <div className="flex items-center gap-2 mb-2">
                        <span className="text-2xl">{m.question.image}</span>
                        <span className="font-bold text-gray-800">
                          {m.question.rawQuestion}
                        </span>
                      </div>
      ...
      <div className="bg-white rounded-3xl shadow-lg p-6 max-w-md w-full border-b-8 border-indigo-200 relative overflow-hidden">
        <div className="text-center mb-6">
          ...
          <div className="relative inline-block mt-2">
            ...
            <button
              ...
              className={`absolute -bottom-5 -right-5 rounded-full p-4 text-white shadow-xl transition-all border-4 border-white ${
                isPlayingAudio
            ...
            <div
              className={`absolute -top-4 -right-4 rounded-full w-14 h-14 flex flex-col items-center justify-center font-black text-xl shadow-lg border-4 border-white transition-colors duration-300 ${
                isTimerRunning
                  ? timeLeft <= 5
        ...
        <div className="space-y-3 mt-6">
          {currentQ.options.map((option, index) => {
            let btnClass =
              'w-full text-left p-4 rounded-2xl text-lg font-bold border-2 transition-all flex items-center justify-between ';
            ...
            if (feedback && isThisOptionCorrect) {
              ...
              btnClass +=
                'bg-white border-gray-200 text-gray-700 hover:border-indigo-400 hover:bg-indigo-50 hover:shadow';
            }
              ...
              >
                ...
                <div
                  className={`p-2 rounded-full transition-colors ${
                    feedback
                      ? 'hidden'
                      : 'text-indigo-300 hover:text-indigo-600 hover:bg-indigo-100'
                  }`}
                  onClick={(e) => {
                    e.stopPropagation();
                    speakText(
                      currentQ.playOptionTexts[index],
                      0.85,
                      currentQ.id
                    );
                  }}
          ...
          <div className="absolute inset-0 flex items-center justify-center bg-white/90 backdrop-blur-sm z-10 animate-in fade-in zoom-in duration-300">
            {feedback === 'correct' ? (
              <div className="text-center transform scale-110">
                ...
                <h3 className="text-4xl font-black text-green-600 tracking-wider">
                  答對了！
                </h3>
              </div>
            ) : (
              <div className="text-center transform scale-110">
                ...
                <h3 className="text-4xl font-black text-red-600 tracking-wider">
                  再接再厲！
                </h3>
              </div>
            )}
